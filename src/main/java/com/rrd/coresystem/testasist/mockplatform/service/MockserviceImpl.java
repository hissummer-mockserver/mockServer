...
public class MockserviceImpl {
	 ...
	 * @return mock的报文
	 */
	public String getMockResponse(Map<String, String> headers, String hostname, String requestUri) {

		MockResponse response = __getMatchedMockResponse(headers, hostname, requestUri);

		if (response != null)
			return response.getResponseBody();
		else
			return JSON.toJSONString(MockPlatformHttpResponseBodyVo.builder().status(0).success(false).message("根据请求地址未找到匹配的mock规则").build());
	...
	public boolean addMockRule(String hostName, String requestUri, String mockResponse,
		...
		try {
			...
			Document addmatachedResult = dataplatformServiceImpl
					.getDocumentByRunCommand(__generateInsertMockRuleCommand(hostNameFormat, requestUriFormat, mockResponse));
	...
	public boolean updateMockRule(String id, String hostName, String requestUri, String mockResponse,
		...
		try {
			...
			Document addmatachedResult = dataplatformServiceImpl
					.getDocumentByRunCommand(__generateUpdateMockRuleCommand(id,hostName, requestUriFormat, mockResponse));
			// 设置document output 到json的设置
			Builder documentBuilder = JsonWriterSettings.builder();
			documentBuilder.outputMode(JsonMode.EXTENDED);
	 ...
	 * @return
	 */
	private MockResponse __getMatchedMockResponse(Map<String, String> headers, String hostName,String requestUri) {
...
//		updateDocument.put("host", hostName == null || hostName.trim().equals("") ? "undefined" : hostName);
//		updateDocument.put("uri", requestUri);
	 ...
	 * @param hostName 如果传入的为null或者为空字符串,则hostName认为是* 并进行查找
	 * @param requestUri  requestUri 从业务场景来看, 不可能为null或者空字符串. 如果传入,则找不到. (因为添加规则时, 不可能出现uri为null和空串的可能性)
	 * @return
	 */
